...
struct _aubio_pitchyinfft_t
{
  ...
  CFFTWrapper *fft;   /**< fft object to compute square difference function */
...
aubio_pitchyinfft_do (aubio_pitchyinfft_t * p, const fvec_t * input, fvec_t * output)
{
  ...
  float *FFTRe = new float[length/2];
  float *FFTIm = new float[length/2];
  p->fft->FftForward(p->winput->data, FFTRe, FFTIm);
  p->fft->TransCompspec(FFTRe, FFTIm, fftout->data);
  ...
  p->fft->FftForward(p->sqrmag->data, FFTRe, FFTIm);
  p->fft->TransCompspec(FFTRe, FFTIm, fftout->data);
  yin->data[0] = 1.;
